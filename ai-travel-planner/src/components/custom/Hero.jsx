  ...
  return (
    <div className="w-full min-h-screen flex flex-col items-center justify-start overflow-hidden">
      ...
      <section className="py-16 px-4 max-w-6xl mx-auto text-center relative">
        ...
        <div className="grid md:grid-cols-3 gap-8 relative">
          {[
            { icon: <FaSearchLocation className="text-2xl text-white" />, title: "Choose Destination", desc: "Pick where you want to go, anywhere in the world.", gradient: "from-blue-500 to-indigo-600", delay: 0 },
            { icon: <FaCalendarAlt className="text-2xl text-white" />, title: "Set Preferences", desc: "Select budget, days, and who you're traveling with.", gradient: "from-green-500 to-emerald-600", delay: 200 },
            { icon: <FaRobot className="text-2xl text-white" />, title: "AI Generates Plan", desc: "Get a customized day-by-day itinerary instantly.", gradient: "from-purple-500 to-pink-600", delay: 400 },
          ].map((item, idx) => (
            <div key={idx} className={`p-6 rounded-2xl bg-white/95 backdrop-blur-md border border-gray-100/80 shadow-lg hover:shadow-xl transform hover:-translate-y-2 transition-all duration-500`} style={{ transitionDelay: `${item.delay}ms` }}>
              <div className={`w-16 h-16 mx-auto flex items-center justify-center rounded-2xl bg-gradient-to-r ${item.gradient} shadow-lg mb-5 transform hover:scale-110 transition-transform duration-300`}>
                {item.icon}
              </div>
              <h3 className="font-bold text-xl mb-3 text-gray-800">{item.title}</h3>
      ...
      <section className="py-16 px-4 bg-gradient-to-br from-gray-50 to-blue-50/30 relative overflow-hidden">
        <div className="max-w-6xl mx-auto text-center">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {[
              { icon: <FaMapMarkedAlt className="text-2xl text-white" />, title: "Personalized Itineraries", desc: "AI crafts trips just for you based on your budget, time, and style.", gradient: "from-blue-500 to-indigo-600" },
              { icon: <FaUsers className="text-2xl text-white" />, title: "Travel with Friends", desc: "Plan solo or group adventures with ease — everyone's included.", gradient: "from-green-500 to-emerald-600" },
              { icon: <FaRegStar className="text-2xl text-white" />, title: "Trusted & Loved", desc: "Thousands of happy travelers use our planner for stress-free trips.", gradient: "from-amber-500 to-orange-600" },
            ].map((item, idx) => (
              <div key={idx} className="p-6 bg-white/90 backdrop-blur-sm rounded-2xl border border-gray-100/60 shadow-lg hover:shadow-xl transform hover:scale-105 transition-all duration-500">
                <div className={`w-16 h-16 mx-auto flex items-center justify-center rounded-2xl bg-gradient-to-r ${item.gradient} shadow-lg mb-5`}>
                  {item.icon}
                </div>
                <h3 className="font-bold text-xl text-gray-800 mb-3">{item.title}</h3>
          ...
          >
            {[
              ...
              <SwiperSlide key={idx}>
                <div className="flex justify-center">
                  <div className="p-3 sm:p-4 bg-white rounded-lg shadow-md hover:shadow-lg transform transition duration-300 text-center border border-gray-100 w-[90%] sm:w-2/3 md:w-1/2">
                    <img src={review.img} alt={review.name} className="w-14 h-14 sm:w-16 sm:h-16 rounded-full mx-auto mb-2 object-cover border-2 border-indigo-100 shadow-sm" />
                    <p className="text-gray-600 italic text-xs sm:text-sm leading-snug">“{review.text}”</p>
                    <h3 className="mt-2 font-semibold text-gray-900 text-xs sm:text-sm">- {review.name}</h3>
                  </div>
                </div>
      ...
      <section className="py-16 px-4 bg-white relative overflow-hidden">
        <div className="max-w-2xl mx-auto text-center p-8 bg-gradient-to-br from-blue-50 to-indigo-50/50 rounded-3xl shadow-lg border border-blue-100/60">
          <h2 className="text-3xl font-bold mb-4 text-gray-800">Join Our Travel Community</h2>
          <p className="mb-6 text-gray-600">Get <span className="font-semibold text-blue-600">AI itineraries</span> & <span className="font-semibold text-blue-600">travel tips</span> delivered first!</p>
          <form className="flex flex-col sm:flex-row gap-3 justify-center items-stretch">
            <input type="email" placeholder="Enter your email..." className="flex-grow px-5 py-3 rounded-2xl border border-gray-200 focus:ring-2 focus:ring-blue-400 focus:border-transparent outline-none shadow-sm" />
            <button className="bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 px-6 py-3 rounded-2xl font-semibold text-white shadow-md hover:shadow-lg transition-all duration-300">Join Now</button>
          </form>
      ...
      <style>{`
        @keyframes float-slow { 0%,100%{transform:translateY(0)}33%{transform:translateY(-20px) rotate(2deg)}66%{transform:translateY(10px) rotate(-2deg)} }
        @keyframes float-medium { 0%,100%{transform:translateY(0)}33%{transform:translateY(-15px) rotate(-1deg)}66%{transform:translateY(5px) rotate(1deg)} }
        @keyframes float-fast { 0%,100%{transform:translateY(0)}50%{transform:translateY(-10px) rotate(1deg)} }
        .animate-float-slow { animation: float-slow 10s ease-in-out infinite; }
        .animate-float-medium { animation: float-medium 8s ease-in-out infinite; }
        .animate-float-fast { animation: float-fast 6s ease-in-out infinite; }
      `}</style>
